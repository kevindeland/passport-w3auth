...
function Strategy(api) {
	...
	this.api = api;
	this._server = {
			url: config.bluepages.url
	};
	
	this._base = config.bluepages.base;
	this._search = {
			filter: config.bluepages.filter
	};
	
	this._debug = config.bluepages.debug;
	
	this.createClient = function() {
		return ldap.createClient(this._server);
	}
